...
public class KOLTweetsCrawler extends Crawler {
      ...
      public boolean crawl() {
            ...
            for (int tweet_count = 0; tweet_count < Constant.TWEET_COUNT_LIMIT;) {
                  ...
                  List<WebElement> possible_divs = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("article[data-testid='tweet']")));

                  // get the inspected div
                  WebElement inspected_div = possible_divs.get(0);

                  /// Checking
                  // check the tweet height
                  int tweet_height = ((Long) js_executor.executeScript("return arguments[0].offsetHeight;", inspected_div)).intValue();
